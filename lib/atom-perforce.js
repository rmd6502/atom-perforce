...
var InputDialog = require('./atom-input-dialog.js')
...
function getClientList(p4Env, callback) {
    let whoami = os.userInfo().username
    let subprocess = cp.spawn('p4',['clients', '-u', whoami], { env: p4Env })
    var clientText = Buffer.alloc(0)
    subprocess.stdout.on('data', data => {
        clientText = Buffer.concat([clientText, data])
    })
    subprocess.on('close', () => {
        let clientOutput = clientText.toString().split('\n')
        let clientList = []
        for (var clientLine of clientOutput) {
            if (clientLine.startsWith('Client')) {
                let toks = clientLine.split(' ')
                clientList.push(toks[1])
            }
        }
        callback(null, clientList)
    })
    subprocess.on('error', err => { callback(err) })
}

function p4ClientList() {
    return Q.when(environmentReady || atomPerforce.exports.setupEnvironment())
    .then(function(p4Env) {
        return Q.nfcall(getClientList, p4Env)
    })
}
...
atomPerforce.exports = {
    ...
    /**
     * Set the workspace name. Updates P4CLIENT in the node environment.
     */
    setWorkspace: function setWorkspace() {
        ...
        execP4Command('info', { cwd: dir })
        .then(function(p4Info) {
            let currentWorkspace = p4Info['clientUnknown.'] ? '' : p4Info['clientName']
            p4ClientList().then(clientList => {
                var dialog = new InputDialog({
                    labelText: "Enter new workspace",
                    callback: function(workspace) { process.env["P4CLIENT"] = workspace; environmentReady = null; deferred.resolve(true); },
                    defaultText: currentWorkspace
                })
                var selectList = new SelectList({
                    items: clientList,
                    elementForItem: (item) => createElementForItem(item),
                    didChangeSelection: (item) => { dialog.miniEditor.setText(item) }
                })
                dialog.element.appendChild(selectList.element)
                dialog.attach()
            })
        })
        .catch(function(err) {
